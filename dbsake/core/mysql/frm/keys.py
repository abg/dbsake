...
_Key = collections.namedtuple('Key',
...
class Key(_Key):
    ...
    def format(self):
        ...
        if self.parser not in (None, True):
            components.append('/*!50100 WITH PARSER `{0}` */'.format(self.parser))
...
def unpack_key_parts(parts_count, keyinfo, columns):
    for _ in _range(parts_count):
        ...
        offset = keyinfo.uint16() - 1
        flags = keyinfo.uint8()
        key_type = keyinfo.uint16()
...
def unpack_keys(keyinfo, columns, parser_info=None):
    ...
    offset = keyinfo.tell() + key_count * BYTES_PER_KEY + key_parts_count * BYTES_PER_KEY_PART
    ...
    for keyname in names:
        ...
        comment = comments.bytes_prefix16() if flags & HA_USES_COMMENT else None
